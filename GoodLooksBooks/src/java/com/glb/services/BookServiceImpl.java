...
public class BookServiceImpl implements BookService {
    ...
    public List<Book> searchBooks(HashMap<String,String> searchTermMap, ArrayList<String> categories, int offset, int recordsPerPage, 
        ...
        try {
            ...
            results = bookDao.searchBooks(searchTermMap, categories, offset, recordsPerPage, onlyInStock, formats, languages, readingLevels);
   ...
   public int renewItem(String username, String isbn) {
      ...
      try {
          ...
          BookDao bookDao = DaoFactory.getBookDao();
          bookDao.setConnection(conn);
          status = bookDao.renewItem(username, isbn);
      } catch (ResourceHelperException ex) {
          Logger.getLogger(BookDao.class.getName()).log(Level.SEVERE, null, ex);
      } finally {
          ConnectionUtil.closeConnection(conn);
